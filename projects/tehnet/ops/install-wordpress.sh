...
required=(
  TEHNET_DB_NAME TEHNET_DB_USER TEHNET_DB_PASSWORD
  TEHNET_ADMIN_USER TEHNET_ADMIN_PASSWORD TEHNET_ADMIN_EMAIL
)
...
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
  echo "Run with sudo -E so packages and web-root ownership can be configured." >&2
  exit 77
fi
...
apt-get install -y -qq \
  nginx mariadb-server curl ca-certificates rsync unzip \
...
DB_PASS_ESCAPED="$(sql_escape "$TEHNET_DB_PASSWORD")"
mariadb <<SQL
...
if [[ ! -f /var/www/tehnet/wp-load.php ]]; then
  sudo -u www-data wp core download --path=/var/www/tehnet --locale=fa_IR
fi

if [[ ! -f /var/www/tehnet/wp-config.php ]]; then
  sudo -u www-data wp config create \
    --path=/var/www/tehnet \
    --dbname="$TEHNET_DB_NAME" \
    --dbuser="$TEHNET_DB_USER" \
    --dbpass="$TEHNET_DB_PASSWORD" \
    --dbhost=localhost \
    --dbcharset=utf8mb4 \
    --skip-check
...
if ! sudo -u www-data wp core is-installed --path=/var/www/tehnet >/dev/null 2>&1; then
  sudo -u www-data wp core install \
    ...
    --url='https://tehnet.ir' \
    --title='TehNet' \
    --admin_user="$TEHNET_ADMIN_USER" \
    --admin_password="$TEHNET_ADMIN_PASSWORD" \
    --admin_email="$TEHNET_ADMIN_EMAIL" \
    --skip-email
fi
...
echo "TehNet WordPress runtime is installed. Nginx vhost activation is a separate validated step."
