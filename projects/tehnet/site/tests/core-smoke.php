...
$missing=[];
foreach ($required as $file) if (!is_file($plugin.'/'.$file)) $missing[]=$file;
...
foreach (['tn_lab','tn_service','_tehnet_sale_mode','direct','inquiry'] as $needle) {
  if (strpos($all,$needle)===false) { fwrite(STDERR,"Missing required core token: $needle\n"); exit(1); }
}
