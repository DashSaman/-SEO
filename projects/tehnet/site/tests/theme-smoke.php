...
foreach ($required as $file) {
    if (!is_file($base . '/' . $file)) $missing[] = $file;
}
if ($missing) {
    fwrite(STDERR, "Missing theme files: " . implode(', ', $missing) . PHP_EOL);
    exit(1);
}
$front = file_get_contents($base . '/front-page.php');
foreach (['/learn/','/labs/','/services/','/shop/'] as $path) {
    if (strpos($front, $path) === false) {
        fwrite(STDERR, "Missing homepage journey link: $path" . PHP_EOL);
        exit(1);
    }
}
