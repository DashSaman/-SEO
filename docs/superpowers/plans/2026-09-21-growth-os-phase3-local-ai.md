...
- Every site will later receive one isolated daily report every 24 hours.
- Every site will later run a measured optimization cycle every 7 days with before/after evidence.
...
Use the current official Linux installer. Keep it under systemd so it recovers after WSL start. Configure:
- `OLLAMA_MAX_LOADED_MODELS=1`
- `OLLAMA_NUM_PARALLEL=1`
- bounded queue
- local/private bind only; never public Internet exposure

Verification:
- service active
- API version responds
- GPU is detected during inference

### P3-AI-03 — Pull resource-safe local models
Primary: `qwen3:8b` Q4_K_M (~5.2GB model payload), chosen to fit the 8GB GPU while retaining multilingual/agent capability.
Fast fallback: `qwen3:4b` Q4_K_M (~2.5GB).
Record exact model digests/licenses in the license ledger after pull.

Verification:
- both models listed locally
- Persian and English smoke prompts succeed
...
Routes:
- `growth-primary` → qwen3:8b
- `growth-fast` → qwen3:4b
...
- qwen3:8b and qwen3:4b are callable locally;
- LiteLLM exposes a stable OpenAI-compatible local-only gateway;
- a Persian production-style prompt succeeds through the gateway;
